...
public class BlackJackImp implements BlackJackService {
	private static List<CardVO> cardList;
	private static List<PlayerVO> playerList;
	...
	public void calculate(PlayerVO vo) {
		...
		for(int i:aceIndex1) {//Ace 카드 포함시켜서 점수 계산
			...
			//int diffVal1=21-caseOfAce[0];
			//int diffVal2=21-caseOfAce[1];
		...
		vo.setCardSet2(_value1); //2번 카드셋 계산 끝
		...
		for(int i:aceIndex3) {//Ace 카드 포함시켜서 점수 계산
			if(caseOfAce[0]+vo.getCardList3().get(i).getValue()+(--aceIndexSize3)<=21) {
				...
				caseOfAce[1]+=vo.getCardList3().get(i).getValue2();
			}
			int diffVal1=21-caseOfAce[0];
			int diffVal2=21-caseOfAce[1];
			if(diffVal1<diffVal2) {
		...
		vo.setCardSet3(_value1);//3번 카드셋 점수계산 끝
		...
		for(int i:aceIndex4) {//Ace 카드 포함시켜서 점수 계산
			if(caseOfAce[0]+vo.getCardList4().get(i).getValue()+(--aceIndexSize4)<=21) {
				...
				caseOfAce[1]+=vo.getCardList4().get(i).getValue2();
			}
		...
		vo.setCardSet4(_value1);//4번 카드셋 계산 끝*/
	...
	public void hit(PlayerVO playerVO, int cardSet) {
		...
		switch(cardSet) {
			...
			case 2:
				...
				break;
			case 3:
				if(playerVO.getCardList3().get(0).getValue()==0) {
					System.out.println("아직 2번째 카드세트에 split을 사용하지 않아서 hit은 무효가 됩니다.");
					break;
				}
				index=cardList.size()-1;
				...
				break;
			case 4:
				if(playerVO.getCardList4().get(0).getValue()==0) {
					System.out.println("아직 2번째 카드세트에 split을 사용하지 않아서 hit은 무효가 됩니다.");
					break;
				}
				index=cardList.size()-1;
				...
				break;
			default:
				System.out.println(cardSet+"번째 카드셋은 없습니다.");
		}
	}
